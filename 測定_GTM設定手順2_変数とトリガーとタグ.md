...
# 追加分（要件①の「滞在時間との組み合わせ計測」）

コード側は実装して本番稼働中。**追加作業は変数2個・トリガー1個・タグ1個・既存タグに1行だけ**（10分程度）。

## A. 変数を2個追加（Part 1 と同じ手順）

タイプ「データレイヤーの変数」／バージョン2／記号なし

| 入力する名前 | 何が入るか |
|---|---|
| time_on_page_ms | その深さに到達するまでに見ていた時間（ミリ秒） |
| seconds | 滞在の節目（15 / 30 / 60 / 120） |

## B. 既存タグに1行だけ足す

**GA4 - scroll_depth**（作成済みのタグ）を開き、パラメータをもう1行追加:

| パラメータ名（入力） | 値（＋から選ぶ） |
|---|---|
| time_on_page_ms | time_on_page_ms |

これで「25%まで読むのに何秒かけたか」が取れる = **スクロール率と滞在時間の組み合わせ**。

## C. トリガーを1個追加

「トリガー」→ 新規 → カスタムイベント → イベント名: **dwell_time**
名前は `CE - dwell_time`

## D. タグを1個追加

「タグ」→ 新規 → GA4イベント → 測定ID `G-KH6VZ8B6HY`
イベント名: **dwell_time** ／ トリガー: `CE - dwell_time`
...
| 指標名 | イベントパラメータ | 単位 |
|---|---|---|
| 到達までの滞在時間 | time_on_page_ms | ミリ秒 |

## F. プレビューで確認 → 公開

サイトを開いて**15秒以上そのまま置いておく** → `GA4 - dwell_time` が Fired になる。
スクロールできるページ（スマホ幅）で下まで送ると `GA4 - scroll_depth` に
time_on_page_ms が乗る。

> タブを裏に回している間は数えないので、**プレビュー画面ではなくサイトのタブを
> 表に出したまま**15秒待ってください。
